...
from parameter_fitting import simulate_data_for_fitting, grid_search_parameter_fit
...
def parameter_recovery(num_samples=20):
    ...
    recovered_params = {'alpha': [], 'beta': []}

    for i in range(num_samples):
        alpha, beta = true_params['alpha'][i], true_params['beta'][i]
        simulated_data = simulate_data_for_fitting(alpha=alpha, beta=beta)
...
def evaluate_parameter_fit(true_params, recovered_params):
    ...
    return (alpha_corr, beta_corr), (alpha_mae, beta_mae)
